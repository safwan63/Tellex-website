...
export default function GuidedFlow({ type, language, onBack }: Props) {
  ...
  const handleNext = async () => {
    ...
    switch (currentSection) {
      ...
      case 3: 
        ...
        break;
      case 4: isValid = await trigger(["bookType"]); break;
      case 5: isValid = await trigger(["readBooks"]); break;
  ...
  if (orderComplete) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center space-y-8 mt-10 max-w-lg mx-auto">
        <div className="space-y-4">
          <h2 className="text-4xl font-extrabold text-[#0E462B]" style={{ fontFamily: "'Playfair Display', serif" }}>
            {type === "mystery" ? "Order confirmed ✅" : "Your vibe has been received ✨"}
          </h2>
          <div className="h-1 w-20 bg-[#0E462B]/20 mx-auto rounded-full" />
        </div>
          ...
          {type === "mystery" ? (
            <>
              <p className="flex items-start gap-3 text-gray-700">
                ...
                <span>We’re now finding a book that truly matches your current vibe.</span>
              </p>
              <p className="flex items-start gap-3 text-gray-700">
                <span className="text-[#0E462B] mt-1">•</span>
              ...
              <p className="flex items-start gap-3 text-gray-700">
                <span className="text-[#0E462B] mt-1">•</span>
                <span>You’ll receive tracking details once it’s shipped via WhatsApp.</span>
              </p>
            </>
          ) : (
            <>
              <p className="flex items-start gap-3 text-gray-700">
                <span className="text-[#0E462B] mt-1">•</span>
                <span>We’re choosing a book that truly understands your current mood.</span>
              </p>
              <p className="flex items-start gap-3 text-gray-700">
                <span className="text-[#0E462B] mt-1">•</span>
                <span>Prepared within 24 hrs , you’ll get a WhatsApp notification once done</span>
              </p>
            </>
          )}
        </div>
  ...
  return (
    <div className="w-full">
      ...
      <AnimatePresence mode="wait">
        ...
        {/* SECTION 2 - Emotion & Life Situation */}
        {currentSection === 2 && (
          ...
          <motion.div key="sec-7" variants={slideVariants} initial="initial" animate="enter" exit="exit" className="space-y-8">
            ...
            {type === "mystery" && (
              <div className="bg-[#e1cfbc]/30 p-6 rounded-xl text-center border border-[#e1cfbc]">
                <p className="font-bold text-[#0E462B] mb-2">Delivery within {getDeliveryRange()}</p>
                <p className="font-medium text-[#0E462B]/70 text-sm">Book name will stay a mystery until it reaches you 🎁</p>
              </div>
            )}

            {type === "vibe" && (
              <div className="bg-[#ebf8ff] p-4 rounded-xl text-center border border-[#3182ce]/20 space-y-1">
                <p className="text-sm font-semibold text-[#2c5282]">Give us up to 24 hours to prepare it for you,</p>
                <p className="text-xs text-[#2c5282]/80">You’ll be notified once it’s ready via WhatsApp</p>
              </div>
            )}
