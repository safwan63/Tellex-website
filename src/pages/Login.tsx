...
export default function Login() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.dir(err); // Provide interactive object details in console
      if (err.code !== 'auth/popup-closed-by-user') {
        setError(`${err.message} (${err.code})` || 'Google authentication failed');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-xl border border-[#0E462B]/10">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="text-center pt-2">
            <Link to="/signup" className="text-sm text-[#0E462B] hover:underline">
